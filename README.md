...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .settings-btn { background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.08); border-radius:8px; width:30px; height:30px; display:flex; align-items:center; justify-content:center; cursor:pointer; font-size:14px; }
    ...
    .settings-input { flex:1; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); border-radius:10px; padding:10px 12px; color:#fff; font-size:12px; font-family:monospace; }
    ...
    .settings-close { width:100%; padding:12px; border-radius:12px; border:1px solid rgba(255,255,255,0.1); background:transparent; color:rgba(255,255,255,0.4); font-size:14px; cursor:pointer; margin-top:16px; }
    .worker-status { display:flex; align-items:center; gap:6px; font-size:11px; margin-bottom:12px; }
    .status-dot { width:8px; height:8px; border-radius:50%; flex-shrink:0; }
    .test-box { margin-top:14px; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.07); border-radius:10px; padding:12px; }
    .test-result { font-size:11px; font-family:monospace; margin-top:8px; padding:8px; border-radius:6px; background:rgba(0,0,0,0.3); line-height:1.5; word-break:break-all; min-height:36px; color:rgba(255,255,255,0.5); }
    .test-btn { width:100%; padding:10px; border-radius:8px; border:none; background:rgba(96,165,250,0.15); color:#60A5FA; font-size:13px; font-weight:600; cursor:pointer; }
  </style>
</head>
<body>
...
async function processImage(file) {
  ...
  try {
    ...
    previewUrl = dataUrl;
    await callWorker(base64, 'image/jpeg');
  } catch(err) {
...
async function callWorker(base64, mediaType) {
  if (!workerUrl) {
...
{"label":"short type","emoji":"single emoji","description":"max 80 chars","calories":number,"protein":number,"carbs":number,"fat":number}`,
    ...
    try { data = JSON.parse(rawText); } catch {
      errorMsg = 'Unexpected response from worker';
...
async function testConnection() {
  ...
  if (!el) return;
  if (!workerUrl) { el.textContent = 'No worker URL saved yet.'; return; }
  el.textContent = 'Testing...';
  el.style.color = 'rgba(255,255,255,0.5)';
  try {
    const res = await fetch(workerUrl, { method: 'GET' });
    const text = await res.text();
    el.textContent = 'HTTP ' + res.status + ': ' + text.slice(0, 100);
    el.style.color = res.ok ? '#7EE8A2' : '#ff8a8a';
  } catch(err) {
    el.textContent = 'FAILED: ' + (err.message || String(err));
...
async function testPost() {
  ...
  if (!el) return;
  if (!workerUrl) { el.textContent = 'No worker URL saved yet.'; return; }
  el.textContent = 'Sending test POST...';
  el.style.color = 'rgba(255,255,255,0.5)';
  try {
    const res = await fetch(workerUrl, {
      method: 'POST',
    ...
    const text = await res.text();
    el.textContent = 'HTTP ' + res.status + ': ' + text.slice(0, 200);
    el.style.color = res.ok ? '#7EE8A2' : '#ff8a8a';
  } catch(err) {
    el.textContent = 'POST FAILED: ' + (err.message || String(err));
    el.style.color = '#ff8a8a';
  }
}
    ...
    el.innerHTML = `
      <div class="modal-overlay" onclick="if(event.target===this){showReset=false;render();}">
        <div class="modal-box">
          ...
          <div class="modal-body">This will clear all of today's meals.</div>
        ...
        <div class="settings-panel">
          ...
          <div class="worker-status">
            ...
            <span style="color:${workerUrl?'rgba(126,232,162,0.8)':'rgba(255,107,107,0.7)'}">
              ${workerUrl ? `Saved: ${shortUrl}` : 'Not set'}
            </span>
          </div>
          ...
          <div class="test-box">
            <div style="font-size:11px;color:rgba(255,255,255,0.4);margin-bottom:8px;font-weight:600;">🔧 CONNECTION TESTS</div>
            <div style="display:flex;gap:8px;margin-bottom:8px;">
              <button class="test-btn" style="flex:1" onclick="testConnection()">Test GET</button>
              <button class="test-btn" style="flex:1;background:rgba(126,232,162,0.1);color:#7EE8A2" onclick="testPost()">Test POST</button>
            </div>
            <div class="test-result" id="testResult">Tap a test button above to check the connection</div>
          </div>
...
function renderManualForm(container) {
  const f = form, ok = f.calories.trim() !== '';
  container.innerHTML = `
    <div class="form-box">
      ...
      <div class="macro-grid">
        <div class="macro-col"><div class="macro-lbl" style="color:#fff">Cal *</div><input class="macro-input" id="calInput" type="number" inputmode="numeric" placeholder="0" value="${f.calories}" style="color:#fff"></div>
        <div class="macro-col"><div class="macro-lbl" style="color:#7EE8A2">Protein g</div><input class="macro-input" id="proInput" type="number" inputmode="numeric" placeholder="0" value="${f.protein}" style="color:#7EE8A2"></div>
        <div class="macro-col"><div class="macro-lbl" style="color:#60A5FA">Carbs g</div><input class="macro-input" id="carbInput" type="number" inputmode="numeric" placeholder="0" value="${f.carbs}" style="color:#60A5FA"></div>
        <div class="macro-col"><div class="macro-lbl" style="color:#F59E0B">Fat g</div><input class="macro-input" id="fatInput" type="number" inputmode="numeric" placeholder="0" value="${f.fat}" style="color:#F59E0B"></div>
      </div>
      <div class="form-btns">
        <button class="cancel-btn" onclick="cancelForm()">Cancel</button>
        <button class="submit-btn ${ok?'active':'inactive'}" id="submitBtn" onclick="submitForm()">${ok?'✓ Add to Log':'Enter calories to continue'}</button>
      </div>
    </div>`;
  bindFormListeners();
}
...
function bindFormListeners() {
  ...
  const cal=g('calInput'),pro=g('proInput'),carb=g('carbInput'),fat=g('fatInput'),lbl=g('labelInput'),desc=g('descInput');
...
function ring(value, max, color, label) {
  ...
  return `<div class="ring-wrap"><div class="ring-inner"><svg width="68" height="68">
    <circle cx="34" cy="34" r="${r}" fill="none" stroke="rgba(255,255,255,0.07)" stroke-width="5"/>
    <circle cx="34" cy="34" r="${r}" fill="none" stroke="${color}" stroke-width="5" stroke-dasharray="${dash} ${circ}" stroke-linecap="round" style="transform:rotate(-90deg);transform-origin:34px 34px"/>
  </svg><div class="ring-text"><span class="ring-num">${value}</span><span class="ring-unit">g</span></div></div><span class="ring-label">${label}</span></div>`;
}
  ...
  return `<div class="meal-card">
    ...
    <div class="meal-body">
      <div class="meal-top">
        ...
        <div class="meal-cal-row"><span class="meal-cal">${m.calories}</span><span class="meal-kcal">kcal</span><button class="delete-btn" onclick="deleteMeal(${m.id})">×</button></div>
      </div>
...
function openSettings() { showSettings=true; render(); }
